...
//  All API calls go to /api/weather and /api/forecast
//  which are Vercel serverless functions.
//  The OpenWeatherMap API key NEVER touches this file.
// =============================================================
...
async function fetchByCoords(lat, lon) {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Could not get weather for this location');
    lastLat = lat;
    lastLon = lon;
    renderCurrent(data);
...
function renderForecast(data) {
  ...
  const hourlyItems = list.filter(item =>
  ...
  Object.entries(days).slice(0, 5).forEach(([dayStr, items]) => {
    // pick midday entry for representative condition
    const rep  = items.find(i => new Date(i.dt * 1000).getHours() === 12)
    ...
    const dayName = new Date(dayStr).toLocaleDateString('en-GB', { weekday: 'short' });
...
function fmtTemp(c)    { return isCelsius ? `${Math.round(c)}°C` : `${Math.round(toF(c))}°F`; }
function fmtWind(ms)   { return isCelsius ? `${(ms * 3.6).toFixed(1)} km/h` : `${(ms * 2.237).toFixed(1)} mph`; }
